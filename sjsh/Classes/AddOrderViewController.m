...
@property(nonatomic,strong)UIImageView *addressIco;
...
@property(nonatomic,strong)UILabel *submitInfoPreferentialContent;
...
- (void)viewDidLoad {
    ...
    [payWayDictionary setValue:@"icon_order_vip" forKey:@"payWayIcon"];
    [payWayDictionary setValue:@"会员卡" forKey:@"payWayTitle"];
    ...
    payWayDictionary = [[NSMutableDictionary alloc] init];
    [payWayDictionary setValue:@"icon_order_cod" forKey:@"payWayIcon"];
    ...
    self.payTitleView = [[UIView alloc] initWithFrame:CGRectMake(0, self.couponView.frame.origin.y+self.couponView.frame.size.height+10, ScreenWidth, 50)];
    self.payTitleView.backgroundColor = [UIColor whiteColor];
    [self.myScrollView addSubview:self.payTitleView];
    
    
    self.payWayTable = [[UITableView alloc] initWithFrame:CGRectMake(0, self.payTitleView.frame.origin.y+self.payTitleView.frame.size.height, ScreenWidth, 5*60)];
    ...
    self.notesTextView = [[UITextView alloc] initWithFrame:CGRectMake(10, self.payWayTable.frame.origin.y+self.payWayTable.frame.size.height+10, ScreenWidth-20, 100)];
    self.notesTextView.layer.borderWidth = 0.5;
    self.notesTextView.delegate = self;
    self.notesTextView.text = @"备注";
    self.notesTextView.textColor = fontGrayColor;
    ...
    self.voiceView = [[UIView alloc] initWithFrame:CGRectMake(0, self.notesTextView.frame.origin.y+self.notesTextView.frame.size.height+10, ScreenWidth, 60)];
    ...
    //地址**************************
    self.addressIco = [[UIImageView alloc] initWithFrame:CGRectMake(10, 20, 30, 30)];
    self.addressIco.contentMode = UIViewContentModeScaleAspectFit;
    self.addressIco.image = [UIImage imageNamed:@"icon_order_location"];
    [self.addressView addSubview:self.addressIco];
    
    self.addressLabel = [[UILabel alloc] initWithFrame:CGRectMake(50, 5, ScreenWidth-50-50, self.addressView.frame.size.height-10)];
    self.addressLabel.text=@"请选择地址";
    self.addressLabel.textAlignment = NSTextAlignmentLeft;
    self.addressLabel.numberOfLines = 0;
    [self.addressView addSubview:self.addressLabel];
    
    self.addressArrow = [[UIImageView alloc] initWithFrame:CGRectMake(ScreenWidth-35, (self.addressView.frame.size.height-15)/2, 15, 15)];
    ...
    //需支付***********************
    self.payTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, 60, self.payTitleView.frame.size.height)];
    self.payTitleLabel.text=@"需支付";
    //    self.payTitleLabel.textAlignment = NSTextAlignmentCenter;
    [self.payTitleView addSubview:self.payTitleLabel];
    
    self.payTitleContent = [[UILabel alloc] initWithFrame:CGRectMake(80, 0, ScreenWidth-120, self.payTitleView.frame.size.height)];
    ...
    self.payWayLine = [[UIImageView alloc] initWithFrame:CGRectMake(10, self.payTitleView.frame.size.height-1, ScreenWidth-20, 1)];
    self.payWayLine.image = [UIImage imageNamed:@"icon_order_line"];
    self.payWayLine.contentMode = UIViewContentModeScaleToFill;
    [self.payTitleView addSubview:self.payWayLine];
    
    //语音下单***********************
    self.voiceSubmit = [[UIButton alloc] initWithFrame:CGRectMake(10, 10, ScreenWidth-20, 40)];
    self.voiceSubmit.backgroundColor = [UIColor colorWithRed:250.0/255.0 green:99.0/255.0 blue:56.0/255.0 alpha:1.0];
    ...
    self.voiceSubmit.layer.masksToBounds = YES;
    self.voiceSubmit.layer.cornerRadius = 5;
    ...
    self.submitInfoLine = [[UIView alloc] initWithFrame:CGRectMake(10, self.submitInfoPreferentialTitle.frame.origin.y+self.submitInfoPreferentialTitle.frame.size.height+rowMargin, ScreenWidth-20, 1)];
    self.submitInfoLine.backgroundColor = [UIColor grayColor];
    [self.submitInfoView addSubview:self.submitInfoLine];
    
    self.submitInfoPayTitle = [[UILabel alloc] initWithFrame:CGRectMake(10, self.submitInfoPreferentialContent.frame.origin.y+self.submitInfoPreferentialContent.frame.size.height+10, 100, rowHeight)];
    ...
    self.submitInfoPayContent = [[UILabel alloc] initWithFrame:CGRectMake(ScreenWidth-110, self.submitInfoPreferentialContent.frame.origin.y+self.submitInfoPreferentialContent.frame.size.height+10, 100, rowHeight)];
...
-(void)requestMemberSuccess:(ASIHTTPRequest *)request{
    ...
    if ([[dic objectForKey:@"code"] intValue] == 200) {
        ...
        if(self.isMember){//如果是会员卡
            ...
            [payWayDictionary setValue:@"1" forKey:@"payWayState"];
            [payWayDictionary setValue:@"member" forKey:@"payID"];
            self.payWayArray[0] = payWayDictionary;
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    UILabel *nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, ScreenWidth-100, 50)];
    nameLabel.text = myDictionary[@"name"];
    [hView addSubview:nameLabel];
    
    
    UILabel *freightLabel = [[UILabel alloc] initWithFrame:CGRectMake(ScreenWidth-90, 0, 80, 50)];
    ...
    if (section>0) {
        ...
        lineView.image = [UIImage imageNamed:@"icon_order_line"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView.tag==198811) {
        static NSString *reusableIdentifier = @"OrderCommodityCell";
        
        OrderCommodityCell *cell = [tableView dequeueReusableCellWithIdentifier:reusableIdentifier];
        
        if (!cell)
        {
            cell = [[OrderCommodityCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reusableIdentifier];
...
- (void)updateSubmitAlert:(int) type{
    ...
    if (type==1) {
        ...
        self.submitInfoPayContent.text = [NSString stringWithFormat:@"%.2f",self.orderCount+self.commodityTransportationExpenses-self.preferentialMoney];
    }
}
...
-(void)updateLayout{
    ...
    self.payTitleView.frame = CGRectMake(0, self.couponView.frame.origin.y+self.couponView.frame.size.height+10, ScreenWidth, 50);
    
    self.payWayTable.frame = CGRectMake(0, self.payTitleView.frame.origin.y+self.payTitleView.frame.size.height, ScreenWidth, 5*60);
    
    self.notesTextView.frame = CGRectMake(20, self.payWayTable.frame.origin.y+self.payWayTable.frame.size.height+20, ScreenWidth-40, 90);
    
    self.voiceView.frame = CGRectMake(0, self.notesTextView.frame.origin.y+self.notesTextView.frame.size.height+20, ScreenWidth, 60);
    
    self.myScrollView .contentSize = CGSizeMake(ScreenWidth, self.addressView.frame.size.height+self.commodityTable.frame.size.height+self.couponView.frame.size.height+self.payTitleView.frame.size.height+self.payWayTable.frame.size.height+self.voiceView.frame.size.height+self.notesTextView.frame.size.height+10+10+20+20+60);
...
- (void)payOrderByAlipay:(NSDictionary *)myDictionary{
    ...
    myOrder.amount = [NSString stringWithFormat:@"%f",self.orderCount+self.commodityTransportationExpenses-self.preferentialMoney];  //商品价格
...
- (void)payOrderByWX:(NSDictionary *)myDictionary{
    ...
    weChatPayDictionary[@"total_fee"] = [NSString stringWithFormat:@"%.0f",(self.orderCount+self.commodityTransportationExpenses-self.preferentialMoney)*100];
    ...
    product.total_fee = (self.orderCount+self.commodityTransportationExpenses-self.preferentialMoney)*100;
