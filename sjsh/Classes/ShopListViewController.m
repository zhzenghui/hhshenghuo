...
@interface ShopListViewController() <CategoryItemDelegate>
{
    UIButton *button1;
    UIButton *button2;
    UIButton *button3;
...
- (void)viewDidLoad
{
    ...
    //类别的分割线
    UIImageView *shadowImage = [[UIImageView alloc] initWithFrame:CGRectMake(59, 40, 3, kScreenBounds.size.height - 44.0f-44)];
    ...
    //左侧类别滚动列表
    self.categoryScroll = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 40, 59, kScreenBounds.size.height -44-44.0f-40)];
    ...
    listTableView = [[UITableView alloc] initWithFrame:CGRectMake(62, 38+40, kScreenBounds.size.width-62, kScreenBounds.size.height - 44-44.0f-38) style:UITableViewStylePlain];
...
- (void)addCategoryItem
{
    ...
    for (NSDictionary *Item in self.categoryListArray) {
        ...
        CategoryItem *leftitem = [[CategoryItem alloc] initWithFrame:rect Image:[UIImage imageNamed:[Item objectForKey:@"img"]] selectedImage:[UIImage imageNamed:[Item objectForKey:@"img_on"]] title:[Item objectForKey:@"title"] index:i];
...
-(void)requestProductListSuccess:(ASIHTTPRequest *)request{
    ...
    //修改列表状态
    for (CategoryItem *item in _categoryScroll.subviews) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView.tag == 198802) {
        ...
        if (indexPath.row == [self.dataArray count] && [self.dataArray count] > 0) {
            ...
            shopListViewCell *cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
            if (cell == nil) {
                cell = [[[shopListViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier] autorelease];
            ...
            if([self.dataArray count]>indexPath.row){
                ...
                if ([dataDic objectForKey:@"member_price"]) {
                     [cell.memberPriceLabel setText:[NSString stringWithFormat:@"会员价%@",[dataDic objectForKey:@"member_price"]]];
                }else{
                    cell.memberPriceLabel.hidden = YES;
                }
...
//    [infoDictionary setObject:ascend?@"asc":@"desc" forKey:@"order"];
//    [infoDictionary setValue:[NSString stringWithFormat:@"%d",page] forKey:@"page"];
//    [infoDictionary setValue: self.searchedContent forKey:@" keywords"];
//    [commonModel getCommoditySearch:infoDictionary httpRequestSucceed:@selector(requestProductListSuccess:) httpRequestFailed:@selector(requestProductListFailed:)];
