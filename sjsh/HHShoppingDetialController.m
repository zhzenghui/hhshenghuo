...
@property (nonatomic, strong)  UILabel *salesLable;    //已售数量
...
@property (nonatomic, strong)  UIImageView *aftermarketIco;    //售后
@property (nonatomic, strong)  UILabel *aftermarketLabel;
...
@implementation HHShoppingDetialController
...
//    [self.view addSubview:self.patButton];
    ...
    self.bottomView = [[UIView alloc]initWithFrame:CGRectMake(0, ScreenHeight-64-50, ScreenWidth, 50)];
    ...
    self.addCartButton = [[UIButton alloc]initWithFrame:CGRectMake(ScreenWidth*0.5, 0, ScreenWidth*0.5, 50)];
...
//    self.addCartButton.layer.masksToBounds = YES;
    ...
    self.collectButton = [[UIButton alloc]initWithFrame:CGRectMake(15, 0, 80, 50)];
    ...
    //页面整体滚动视图
    self.pageScrollView=[[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight-64-50)];
    self.pageScrollView.contentSize=CGSizeMake(self.pageScrollView.bounds.size.width, 1000);
    ...
    //图片滚动栏
    self.commodityScroll=[[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenWidth)];
    ...
    self.bannerPageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, self.commodityScroll.frame.size.height-25, ScreenWidth, 20)];
    ...
    [self.bannerPageControl setValue:[UIImage imageNamed:@"hh_dot_biue_ico"] forKey:@"_currentPageImage"];
    [self.bannerPageControl setValue:[UIImage imageNamed:@"hh_dot_white_ico"]  forKey:@"_pageImage"];
...
//    [self.pageScrollView addSubview:self.lineImageView];
    ...
    self.commodityNameLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, self.commodityScroll.frame.origin.y+self.commodityScroll.frame.size.height+15, ScreenWidth-40, 0)];
    ...
    self.commodityDescribeLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, self.commodityNameLabel.frame.origin.y+self.commodityNameLabel.frame.size.height+10, ScreenWidth-30, 0)];
    //自动折行设置
    ...
    self.nowMoneyLable = [[UILabel alloc] initWithFrame:CGRectMake(15.5, 15, 80, 15)];
    self.nowMoneyLable.text = @"￥0元";
    self.nowMoneyLable.textColor = hhRedColor;
    ...
    UIView *oldMoneyFlag = [[UIView alloc] initWithFrame:CGRectMake(95, self.oldMoneyLable.frame.origin.y+self.oldMoneyLable.frame.size.height-7, 60, 1)];
    oldMoneyFlag.backgroundColor = kGrayColor;
    [self.priceInfoView addSubview:oldMoneyFlag];
    
//    self.salesLable = [[UILabel alloc] initWithFrame:CGRectMake(ScreenWidth-100, self.oldMoneyLable.frame.origin.y, 80, 15)];
//    self.salesLable.text = @"已售0";
//    self.salesLable.textColor = fontDilutedGrayColor;
//    self.salesLable.textAlignment = NSTextAlignmentRight;
//    self.salesLable.font = [UIFont systemFontOfSize:14];
//    [self.priceInfoView addSubview:self.salesLable];
 
    
    self.specificationsTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, self.priceInfoView.frame.origin.y+self.priceInfoView.frame.size.height, ScreenWidth, 0)];
    ...
    //送货信息区域
    self.deliverInfoView = [[UIView alloc]initWithFrame:CGRectMake(0, self.specificationsTableView.frame.origin.y+self.specificationsTableView.frame.size.height, ScreenWidth, 65)];
    self.deliverInfoView.backgroundColor = backageYellowColorWithHH;
    ...
    self.changeView.backgroundColor = dilutedGrayColor;
    ...
    self.detailDescribeWeb = [[WKWebView alloc] initWithFrame:CGRectMake(0, self.changeView.frame.origin.y+self.changeView.frame.size.height, ScreenWidth, 500)];
...
- (void)setinforWithDic:(NSDictionary *)dic
{
    ...
    if([dic objectForKey:@"member_price"]&&[[dic objectForKey:@"member_price"] floatValue]>0){
        self.salesLable.text = [NSString stringWithFormat:@"已售￥%.0f",[[dic objectForKey:@"member_price"] floatValue]];
    }else{
        self.salesLable.hidden = YES;
    }
...
- (void) changePage:(UIButton *) myButton{
    switch (myButton.tag) {
        case 198801:
            ...
            self.pageScrollView.contentSize=CGSizeMake(self.pageScrollView.bounds.size.width, 1000+self.specificationsTableView.frame.size.height);
...
- (void)UPdateViewsHeight:(NSObject *)responseData {
    ...
    self.specificationsTableView.frame = CGRectMake(0, self.priceInfoView.frame.origin.y+self.priceInfoView.frame.size.height, ScreenWidth, [responseString integerValue]>0?0+50* [responseString integerValue]:0);
    
    
    self.deliverInfoView.frame = CGRectMake(0, self.specificationsTableView.frame.origin.y+self.specificationsTableView.frame.size.height, ScreenWidth, 70);
    ...
    self.detailDescribeWeb.frame = CGRectMake(0, self.changeView.frame.origin.y+self.changeView.frame.size.height, ScreenWidth, 500);
...
//    if (self.appraiseTableView.frame.size.height>100) {
//        CGRect detailDescribeWebFrame =  self.detailDescribeWeb.frame;
//        detailDescribeWebFrame.size.height = self.appraiseTableView.frame.size.height;
//        self.detailDescribeWeb.frame = detailDescribeWebFrame;
//    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (tableView.tag==198811) {
        ...
        HHSpecificationsCell *cell = [tableView dequeueReusableCellWithIdentifier:reusableIdentifier];
        
        if (!cell)
        {
            cell = [[HHSpecificationsCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reusableIdentifier];
            cell.selectionStyle = UITableViewCellSelectionStyleNone;
        }
        if (indexPath.row == 0) {
            cell.specificationsTitle.hidden = NO;
        }
